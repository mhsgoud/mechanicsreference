...
try{
const response=await fetch(asset('frames.json.gz'));if(!response.ok)throw Error('Could not load three-point bending animation frames');
...
// Reflect the quarter model about its two constrained symmetry planes.
const copies=[];
for(const [sx,sy] of [[-1,1],[1,-1],[-1,-1]]){
 ...
 for(const o of [m,e,g]){o.scale.set(sx,sy,1);scene.add(o);}copies.push({m,e,g});
}
...
function fitBounds(){const box=new THREE.Box3();for(let i=0;i<d.reference.length;i+=3){const v=new THREE.Vector3(...d.reference.slice(i,i+3));box.expandByPoint(v);if($('fullBeam').checked)box.expandByPoint(new THREE.Vector3(-v.x,-v.y,v.z));}box.getCenter(center);radius=box.getSize(new THREE.Vector3()).length()/2;controls.minDistance=radius*.1;controls.maxDistance=radius*20;camera.far=radius*40;camera.updateProjectionMatrix();}
...
function fullBeam(){geo.setIndex($('fullBeam').checked?fullIndex:reducedIndex);geo.computeVertexNormals();syncVisibility();fitBounds();view([.35,-1.8,.65]);}
...
function showTime(t){simTime=t;updateResponse(t);let b=frames.findIndex(f=>f.time>=t);if(b<0)b=frames.length-1;const a=Math.max(0,b-1),A=frames[a],B=frames[b],mix=a===b?0:(t-A.time)/(B.time-A.time);for(const [key,target] of [['u','displacement'],['stress','stress'],['plastic','plastic']])if(A[key]&&d[target])for(let i=0;i<A[key].length;i++)d[target][i]=A[key][i]+mix*(B[key][i]-A[key][i]);for(let i=0;i<d.nodes;i++)d.magnitude[i]=Math.hypot(...d.displacement.slice(3*i,3*i+3));deform();color();$('timeline').value=t;$('frameLabel').textContent=`t = ${t.toFixed(3)} · ${b+1} / ${frames.length}`;$('info').textContent=`7,072 surface triangles · ${frames.length} solved increments`;} 
...
window.biegungViewer={state:()=>({fullBeam:$('fullBeam').checked,instances:$('fullBeam').checked?4:1,scale:+$('scale').value,field:$('field').value,triangles:d.triangles.length/3,ghost:ghost.visible,time:simTime,playing,frames:frames.length}),positions:()=>Array.from(geo.attributes.position.array)};
