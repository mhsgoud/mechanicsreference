...
try{
const response=await fetch(asset('frames.json.gz'));if(!response.ok)throw Error('Could not load steel joint animation frames');
...
const renderer=new THREE.WebGLRenderer({antialias:true,alpha:true,logarithmicDepthBuffer:true});renderer.setPixelRatio(Math.min(devicePixelRatio,2));renderer.setClearColor(0,0);renderer.outputColorSpace=THREE.SRGBColorSpace;$('canvas').appendChild(renderer.domElement);
...
const material=new THREE.MeshStandardMaterial({color:0xc9dbe2,roughness:.38,metalness:.16,side:THREE.FrontSide,polygonOffset:true,polygonOffsetFactor:1,polygonOffsetUnits:1});const mesh=new THREE.Mesh(geo,material);scene.add(mesh);
...
const edges=new THREE.LineSegments(edgeGeometry(d.points),new THREE.LineBasicMaterial({color:0x223e51,transparent:true,opacity:.4,depthWrite:false}));scene.add(edges);edges.visible=$('mesh').checked;const ghost=new THREE.LineSegments(edgeGeometry(d.reference),new THREE.LineBasicMaterial({color:0xeabe78,transparent:true,opacity:.3,depthWrite:false}));ghost.visible=false;scene.add(ghost);
const center=new THREE.Vector3();const box=new THREE.Box3();for(const a of [d.reference,d.points])for(let i=0;i<a.length;i+=3)box.expandByPoint(new THREE.Vector3(...a.slice(i,i+3)));box.getCenter(center);const radius=box.getSize(new THREE.Vector3()).length()/2;camera.near=radius*.001;camera.far=radius*40;camera.updateProjectionMatrix();controls.maxDistance=radius*20;controls.minDistance=radius*.1;
function syncVisibility(){}
function view(direction){controls.target.copy(center);camera.position.copy(center).add(new THREE.Vector3(...direction).normalize().multiplyScalar(radius/Math.sin(THREE.MathUtils.degToRad(camera.fov/2))*1.15));controls.update();}
...
document.querySelector('.note').textContent=`OOFEM results · ${frames.length} saved states. Default ×2 matches the supplied export. True scale ×1 removes amplification. The run stopped before its requested 80 steps.`;
...
function showTime(t){simTime=t;updateResponse(t);let b=frames.findIndex(f=>f.time>=t);if(b<0)b=frames.length-1;const a=Math.max(0,b-1),A=frames[a],B=frames[b],mix=a===b?0:(t-A.time)/(B.time-A.time);for(const [key,target] of [['u','displacement'],['stress','stress'],['plastic','plastic']])if(A[key]&&B[key]&&d[target])for(let i=0;i<A[key].length;i++)d[target][i]=A[key][i]+mix*(B[key][i]-A[key][i]);for(let i=0;i<d.nodes;i++)d.magnitude[i]=Math.hypot(...d.displacement.slice(3*i,3*i+3));deform();color();$('timeline').value=t;$('frameLabel').textContent=`t = ${t.toFixed(3)} · ${b+1} / ${frames.length}`;$('info').textContent=`${d.triangles.length/3} surface triangles · ${frames.length} solved increments`;} 
